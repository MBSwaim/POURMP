...
export function getPrepOutputsData(eventId: number): PrepOutputsData | null {
  ...
  const ev: EventForNotes = {
    ...
    total_event_value: details?.total_event_value ?? null,
    deposit_due: details?.deposit_due ?? null,
    deposit_received: details?.deposit_received ?? null,
    final_amount_due: details?.final_amount_due ?? null,
