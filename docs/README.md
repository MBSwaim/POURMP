...
Toast remains the official system of record for leads, proposals, invoices, deposits, payments, and customer-facing communication. POURMP does not process payments, send invoices, generate proposals, or store financial transactions as authoritative. Instead, POURMP transforms event information into operational plans, task management, preparation documents, leadership visibility, and consistent event execution. Concretely, it:

- **Mirrors** key Toast milestones manually (the Toast Status Tracker: Proposal Sent → Confirmed → Invoice Sent → Deposit Received → Final Payment) so staff can answer "where are we with this client in Toast?" without opening Toast.
- **Mirrors** financial figures (deposit/final due and received) for internal visibility on the Operational Dashboard and Pre-Shift Brief — again, a manual reflection of Toast, not a replacement for it.
- **Projects** sales figures (guest count × package price) for dashboard and analytics purposes as an internal planning proxy — not Toast's actual invoiced or collected revenue.
- Generates the **Toast Notes** Prep Doc specifically as a formatted block of text staff copy directly into Toast's own event notes field, so Toast still receives the operational detail it needs without staff re-typing it there by hand.

If Toast and POURMP ever disagree on a financial number, **Toast is correct.** POURMP's financial fields are a staff convenience, not an accounting system.

*(POURMP's own public inquiry form at `/book` is a lightweight intake that hands new bookings off into an event record — it is not a leads/CRM system and does not compete with Toast's own lead handling.)*
...
- **Analytics** — year-over-year revenue tracking (invoiced vs. collected)
...
POURMP has not yet reached `1.0`. See [ROADMAP.md](ROADMAP.md) for what's required before an official internal rollout.
